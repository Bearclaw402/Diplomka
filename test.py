...
def leNetExp():
  cnn = CNN(10, loss='categorical_crossentropy')
  conv1 = Conv(1,6,5,initializer='xavier_uniform',activation='tanh')
  pool1 = Pool(2,2,'avg')
  conv2 = Conv(6,16,5,initializer='xavier_uniform',activation='tanh')
  pool2 = Pool(2,2,'avg')
  conv3 = Conv(16,120,4,initializer='xavier_uniform',activation='tanh')
  fc1 = Dense(84, 120, initializer='xavier_uniform',activation='tanh')
  sm = Softmax(10, 84,initializer='xavier_uniform')
  cnn.add(conv1)
  cnn.add(pool1)
  cnn.add(conv2)
  cnn.add(pool2)
...
def alexNet():
  ...
  fc1 = Dense(128, 400,initializer='xavier_uniform',activation='relu')
...
def expTest():
    ...
    train_images, train_labels, test_images, test_labels = prepareData(525, True)
    # test_images = mnist.test_images()[:250]
    # test_labels = mnist.test_labels()[:250]
    ...
    for i in range(k):
        ...
        for epoch in range(30):
            ...
            for step in range(steps):
                ...
                loss += l
                num_correct += acc
                # print('[Step %d] Loss %.3f | Accuracy: %.2f%%' % (samples, l, acc * 100.0))
                ll += l
                aa += acc
                # print("ž")
                if (step+1) % 10 == 0:
                    print('%d;%.3f;%.2f%%' % ((step+1), ll / 10.0, aa * 10.0))
                    ll = 0.0
                    aa = 0.0
            # print('Past %d steps: Average Loss %.3f | Accuracy: %.2f%%' % (samples, loss / steps, (num_correct / steps)* 100.0))
            # print('%d;%.3f;%.2f%%' % (samples, loss / steps,  (num_correct / steps)* 100.0))
            # print('False Negative %d' % (stats[1, 0]))
        ...
        #     ti = numpy.expand_dims(test_images[i], 0)
        #     tl = numpy.expand_dims(test_labels[i], 0)
        _, loss, num_correct = cnn.forwardBatch(test_images, test_labels)
        # _, loss, num_correct = cnn.forwardBatch(ti, tl)
        total_stats+=num_correct
        acc = num_correct
        total_loss += loss
        total_acc += acc
        ...
        # print('Test Accuracy:', num_correct)
        print('Test Accuracy:', total_acc * 100.0)
        # total_loss += loss
        # total_acc += num_correct
        # total_acc += acc
        # print("TN  |  FP")
